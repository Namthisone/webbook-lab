...
if($mode==='vulnerable'){
  ...
  try{$q="SELECT id, ten_sach, gia_ban FROM sach WHERE id = '$id'"; $rows=$ket_noi->query($q)->fetchAll(PDO::FETCH_ASSOC); $out=htmlspecialchars(print_r($rows,true));}
  catch(Throwable $e){$out=htmlspecialchars($e->getMessage());}
}else{
  try{$st=$ket_noi->prepare('SELECT id, ten_sach, gia_ban FROM sach WHERE id = ?'); $st->execute([(int)$id]); $out=htmlspecialchars(print_r($st->fetchAll(PDO::FETCH_ASSOC),true));}
