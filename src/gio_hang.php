...
/* Keep the existing product-page links working: only the non-destructive add action may arrive via GET. */
if (($_GET['hanh_dong'] ?? '') === 'them') {
    $id_sach = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
...
/* Destructive cart operations are POST + CSRF protected. */
...
<div class="container"><h2>🛒 Giỏ Hàng Của Bạn</h2>
...
<div class="action-buttons"><form method="post"><input type="hidden" name="cart_csrf" value="<?=htmlspecialchars($_SESSION['cart_csrf'],ENT_QUOTES,'UTF-8')?>"><input type="hidden" name="hanh_dong" value="xoa_het"><button class="btn btn-danger" type="submit">🗑️ Xóa sạch giỏ hàng</button></form><a href="thanh_toan.php" class="btn btn-success">💳 Tiến hành đặt hàng & Thuê sách</a></div>
