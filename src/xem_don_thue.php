...
$stmt = $ket_noi->prepare(
    'SELECT id, ngay_thue, trang_thai, ngay_tao_don, tong_tien_thue
     FROM don_hang_thue
     WHERE id_nguoi_dung = ?
...
<h1>📚 Đơn hàng thuê của tôi</h1>
<p><a href="index.php">← Trang chủ</a></p>
<?php if (!$danh_sach): ?><div class="empty">Bạn chưa có đơn thuê nào.</div>
<?php else: ?><table><thead><tr><th>Mã đơn</th><th>Ngày thuê</th><th>Ngày tạo</th><th>Trạng thái</th><th>Tổng tiền</th></tr></thead><tbody>
<?php foreach ($danh_sach as $don): ?><tr>
<td>#<?= (int)$don['id'] ?></td>
<td><?= htmlspecialchars((string)$don['ngay_thue'], ENT_QUOTES, 'UTF-8') ?></td>
<td><?= htmlspecialchars((string)$don['ngay_tao_don'], ENT_QUOTES, 'UTF-8') ?></td>
<td><?= htmlspecialchars((string)$don['trang_thai'], ENT_QUOTES, 'UTF-8') ?></td>
<td><?= number_format((float)$don['tong_tien_thue'],0,',','.') ?> đ</td>
